    ...
    <div className="page min-h-screen flex flex-col">
      ...
      <header className="px-4 sm:px-5 pb-3 page-header flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <ThemeToggle />
          <p className="text-[13px] font-semibold" style={{ color: 'var(--tx)' }}>{time}</p>
        ...
        {[
          ...
          <button key={t.id} onClick={() => { setTab(t.id); if (t.id === TAB.WORK) reset() }}
            ...
            style={tab === t.id
              ...
              : { background: 'var(--bg-el)', color: 'var(--tx-3)', border: '1px solid var(--bd)' }
            }>
            {t.label}
          </button>
        ))}
          ...
          {step === S.SELECT && (
            <main className="flex-1 flex flex-col px-4 pb-8">
              <div className="mb-5">
                ...
                <p className="text-[13px] mt-1" style={{ color: 'var(--tx-4)' }}>Шаг 1 из 2</p>
              </div>
              ...
              {loading ? (
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-center">
                  ...
                  <p className="font-medium" style={{ color: 'var(--tx-3)' }}>Услуги не настроены</p>
                    ...
                    return (
                      <button key={s.id} onClick={() => setSel(s)}
                        ...
                        style={{
                          ...
                          ...(on ? { background: 'var(--bg-el)', border: '1px solid var(--bd-3)' } : {})
                        }}>
                        <div className="flex items-center gap-3">
                          <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0"
                            style={{ background: on ? 'var(--bd-3)' : 'var(--bg-deep)' }}>
                            {on
                              ? <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--tx)" strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg>
                              : <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--ic)" strokeWidth="1.8"><path d="M6 2v12M6 14c0 2.2 1.8 4 4 4h4a4 4 0 0 0 0-8H6"/><path d="M18 2v12"/></svg>
                            }
                          </div>
                          <span className="text-[15px] font-semibold" style={{ color: on ? 'var(--tx)' : 'var(--tx-2)' }}>
                            {s.name}
                          </span>
                        </div>
                        <div className="text-right">
                          <p className="text-[17px] font-bold" style={{ color: on ? 'var(--tx)' : 'var(--tx-3)' }}>
                            {Number(s.price).toLocaleString()}
                          </p>
                          <p className="text-[11px]" style={{ color: 'var(--tx-5)' }}>сом</p>
                        </div>
                      </button>
                    )
                  })}
              ...
              {sel && (
                <div className="mt-4 asi">
                  <div className="rounded-xl px-4 py-3 mb-3 flex items-center justify-between"
                    style={{ background: 'var(--bg-card)', border: '1px solid var(--bd)' }}>
                    <span className="text-[14px]" style={{ color: 'var(--tx-3)' }}>{sel.name}</span>
                    <span className="text-[15px] font-bold" style={{ color: 'var(--tx)' }}>{Number(sel.price).toLocaleString()} сом</span>
                  </div>
          ...
          {step === S.PAY && (
            <main className="flex-1 flex flex-col px-4 pb-8">
              ...
              <div className="mb-5">
                ...
                <p className="text-[13px] mt-1" style={{ color: 'var(--tx-4)' }}>Шаг 2 из 2</p>
              </div>

              <div className="rounded-xl px-4 py-4 mb-4 flex items-center justify-between"
                style={{ background: 'var(--bg-card)', border: '1px solid var(--bd)' }}>
                <div>
                  <p className="text-[11px] mb-1" style={{ color: 'var(--tx-4)' }}>Услуга</p>
                  <p className="text-[15px] font-semibold" style={{ color: 'var(--tx)' }}>{sel?.name}</p>
                </div>
                <div className="text-right">
                  <p className="text-[24px] font-black leading-none" style={{ color: 'var(--tx)' }}>{Number(sel?.price).toLocaleString()}</p>
                ...
                {[
                  ...
                  return (
                    <button key={p.id} onClick={() => setPay(p.id)}
                      ...
                      style={on ? { background: 'var(--bg-el)', border: '1px solid var(--bd-3)' } : {}}>
                      <div className="w-12 h-12 rounded-2xl flex items-center justify-center flex-shrink-0"
                        style={{
                          background: on ? 'var(--bd-2)' : 'var(--bg-deep)',
                          color: on ? 'var(--tx)' : 'var(--ic)'
                        }}>
                        {p.icon}
                      </div>
                      <div className="flex-1">
                        <p className="text-[16px] font-semibold" style={{ color: on ? 'var(--tx)' : 'var(--tx-2)' }}>
                          {p.label}
                        </p>
                      ...
                      <div className="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0"
                        style={{
                          border: `2px solid ${on ? 'var(--tx-2)' : 'var(--bd-3)'}`,
                          background: on ? 'var(--bd-2)' : 'transparent'
                        }}>
                        {on && <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="var(--tx)" strokeWidth="3"><polyline points="20 6 9 17 4 12"/></svg>}
                      </div>
                    </button>
                  )
                })}
              ...
              <div className="w-full max-w-xs text-center">
                <div className="w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6 ach"
                  ...
                  <svg width="36" height="36" viewBox="0 0 24 24" fill="none"
                    stroke="var(--tx-2)" strokeWidth="2.2" strokeLinecap="round">
                ...
                <div className="rounded-2xl overflow-hidden mb-6"
                  ...
                  {[
                    ...
                    <div key={row.l} className="flex items-center justify-between px-4 py-3"
                      ...
                      <span className={`text-[14px] ${row.bold ? 'font-bold' : 'font-medium'}`}
                        style={{ color: row.bold ? 'var(--tx)' : 'var(--tx-2)' }}>
                        {row.v}
                      </span>
                    </div>
                  ))}
      ...
      {(tab === TAB.SERVICES || !canCreateAppointments) && (
        <main className="flex-1 flex flex-col px-4 pb-8">
          ...
          {editingService && (
            <button onClick={() => setEditingService(null)} className="btn-outline mb-4">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                <line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/>
              </svg>
              Отмена
          ...
          {loading ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center py-16">
              <p className="font-medium" style={{ color: 'var(--tx-3)' }}>Услуг пока нет</p>
              <p className="text-sm mt-1" style={{ color: 'var(--tx-5)' }}>Добавьте первую услугу выше</p>
            </div>
          ) : (
              ...
              {services.map((s, i) => (
                <div key={s.id} className="glass-card afu flex items-center justify-between"
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-[15px]" style={{ color: 'var(--tx)' }}>{s.name}</p>
                    <p className="text-[13px] mt-[2px]" style={{ color: 'var(--tx-3)' }}>
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <button
                      ...
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--ic-2)" strokeWidth="2">
